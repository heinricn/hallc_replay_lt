...
//normalizes data to first data point, if required
void normalizeData ( Double_t *Param[nPlanes][nBarsMax][nSides], Double_t *ParamErr[nPlanes][nBarsMax][nSides], UInt_t numRuns)
{
	...
	for (UInt_t iside = 0; iside < nSides; iside++)
	{
		...
		for(UInt_t iplanes = 0; iplanes < nPlanes; iplanes++)
		{
			...
			for(UInt_t ibar = 0; ibar < nBarsMax; ibar++)
			{
				//aviod dividing by zero
				if (Param[iplanes][ibar][iside][0] != 0)
				{
					P_norm = Param[iplanes][ibar][iside][0];
				}else{
					P_norm = 1;
				}
				...
				{
					Param[iplanes][ibar][iside][irun] = Param[iplanes][ibar][iside][irun]/P_norm;
					ParamErr[iplanes][ibar][iside][irun] = ParamErr[iplanes][ibar][iside][irun]/P_norm;
				}								
			}
...
void HodoParamCompair ( TString runNums_name, UInt_t numRuns ) //input path to run # file, and the number of runs to look at
{
	...
	for(UInt_t irun = 0; irun < numRuns; irun++)
	{
		...
		//get file
		fileName = Form("../../PARAM/SHMS/HODO/Calibration/phodo_TWcalib_Err_%.0f.param", runs[irun]);
